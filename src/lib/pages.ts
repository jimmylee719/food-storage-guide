...
import { CONTACT_EMAIL, OPERATOR } from './site';
...
const P: Record<PageKey, Record<AnyLocale, StaticPage>> = {
  about: {
    zh: {
      ...
      sections: [
        ...
        { heading: '營運者', body: [`本站由${OPERATOR.nameZh}營運與維護。公司統一編號 ${OPERATOR.taxId}，地址為${OPERATOR.addressZh}。`, `需要進一步洽談內容授權、資料引用或商業合作，請聯絡${OPERATOR.nameZh}：${OPERATOR.siteZh}，或來信 ${CONTACT_EMAIL}。`] },
    ...
    en: {
      ...
      sections: [
        ...
        { heading: 'Operator', body: [`This site is operated and maintained by ${OPERATOR.nameEn} (${OPERATOR.nameZh}), registered in Taiwan under unified business number ${OPERATOR.taxId}, at ${OPERATOR.addressEn}.`, `For content licensing, data reuse or business enquiries, contact ${OPERATOR.nameEn} at ${OPERATOR.siteEn}, or email ${CONTACT_EMAIL}.`] },
    ...
    ja: {
      ...
      sections: [
        { body: ['「食品保存ガイド」は、登録不要で使える無料の食品保存データベースです。食材名を入力すると、常温・冷蔵・冷凍それぞれの保存期間と、正しい保存のしかたが分かります。', '繁体字中国語と英語の 2 言語で公開しています。いずれも機械翻訳ではなく、その言語の読者に向けて書き下ろしています。'] },
        { heading: 'なぜ作ったのか', body: ['国連環境計画（UNEP）の推計では、家庭・小売・外食で毎年およそ 10 億食分の食料が捨てられています。その多くは「まだ食べられるか分からない」という理由で捨てられたものです。', 'ネット上の保存情報は多いものの、出典がなく、内容が食い違うこともよくあります。このサイトはその逆を目指しました。数値はすべて公的データにさかのぼれること、そして両言語で同じ基準を示すことです。'] },
    ...
    zh: {
      ...
      sections: [
        ...
        { heading: '內部交叉檢查', body: ['資料進入網站之前與之後，會通過幾道自動檢查。這些檢查只回報問題，不會自動修改內容——每一個發現都需要人去判斷哪一邊才是對的。'], bullets: [
          'scripts/validate-extra.js 與 scripts/validate-content.js：檢查資料結構與文章欄位是否符合規格，繁體中文與英文是否齊備。',
    ...
    en: {
      ...
      sections: [
        ...
        { heading: 'Internal cross-checks', body: ['Data passes several automated checks before and after it reaches the site. They report; they never silently fix. Every finding needs a person to decide which side of the contradiction is right.'], bullets: [
          'scripts/validate-extra.js and scripts/validate-content.js — schema validation for the data records and article fields, and a check that both languages are present.',
    ...
    ja: {
      ...
      sections: [
        ...
        { heading: '内部の相互チェック', body: ['データはサイトに載る前後で、いくつかの自動チェックを通ります。これらは報告するだけで、黙って修正することはありません。どちらが正しいかは、必ず人が判断します。'], bullets: [
          'scripts/validate-extra.js と scripts/validate-content.js — データと記事のスキーマ検証、および両言語がそろっているかの確認。',
    ...
    es: {
      ...
      sections: [
        ...
        { heading: 'Comprobaciones internas cruzadas', body: ['Los datos pasan varias comprobaciones automáticas antes y después de llegar al sitio. Informan; nunca corrigen en silencio. Cada hallazgo necesita que una persona decida qué lado de la contradicción tiene razón.'], bullets: [
          'scripts/validate-extra.js y scripts/validate-content.js — validación de esquema de los registros y de los campos de los artículos, y comprobación de que están ambos idiomas.',
  ...
  privacy: {
    zh: {
      ...
      sections: [
        ...
        { heading: '資料控管者', body: [`本站由${OPERATOR.nameZh}（統一編號 ${OPERATOR.taxId}，地址：${OPERATOR.addressZh}）營運，該公司即為本站個人資料的控管者。隱私權相關詢問請寄 ${CONTACT_EMAIL}。`] },
    ...
    en: {
      ...
      sections: [
        ...
        { heading: 'Data controller', body: [`This site is operated by ${OPERATOR.nameEn} (${OPERATOR.nameZh}), unified business number ${OPERATOR.taxId}, of ${OPERATOR.addressEn}, which is the data controller for this site. Privacy enquiries: ${CONTACT_EMAIL}.`] },
    ...
    ja: {
      ...
      sections: [
        ...
        { heading: '個人情報の管理者', body: [`当サイトは${OPERATOR.nameZh}（台湾統一編号 ${OPERATOR.taxId}、所在地：${OPERATOR.addressZh}）が運営しており、同社が当サイトの個人情報の管理者です。プライバシーに関するお問い合わせは ${CONTACT_EMAIL} までご連絡ください。`] },
    ...
    es: {
      ...
      sections: [
        ...
        { heading: 'Responsable del tratamiento', body: [`Este sitio está operado por ${OPERATOR.nameEn} (${OPERATOR.nameZh}), número de identificación fiscal ${OPERATOR.taxId}, con domicilio en ${OPERATOR.addressEn}, que es el responsable del tratamiento de los datos. Consultas de privacidad: ${CONTACT_EMAIL}.`] },
  ...
  terms: {
    zh: {
      ...
      sections: [
        { body: [`最後更新：2026 年 9 月。本站由${OPERATOR.nameZh}（統一編號 ${OPERATOR.taxId}）營運，以下條款中的「我們」即指該公司。使用本網站即表示你同意以下條款。`] },
    ...
    en: {
      ...
      sections: [
        { body: [`Last updated: September 2026. This site is operated by ${OPERATOR.nameEn} (${OPERATOR.nameZh}), unified business number ${OPERATOR.taxId}; "we" in these terms means that company. By using this site you agree to the terms below.`] },
    ...
    ja: {
      ...
      sections: [
        { body: [`最終更新：2026 年 9 月。当サイトは${OPERATOR.nameZh}（台湾統一編号 ${OPERATOR.taxId}）が運営しており、本規約における「当方」とは同社を指します。当サイトを利用された時点で、以下の条件に同意したものとみなします。`] },
    ...
    es: {
      ...
      sections: [
        { body: [`Última actualización: septiembre de 2026. Este sitio está operado por ${OPERATOR.nameEn} (${OPERATOR.nameZh}), número de identificación fiscal ${OPERATOR.taxId}; «nosotros» en estos términos se refiere a esa empresa. Al usar este sitio aceptas las condiciones siguientes.`] },
