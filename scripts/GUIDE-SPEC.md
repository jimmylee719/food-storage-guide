...
5. Internal linking hints: where relevant mention foods in the form `[[food:slug]]` (e.g. `[[food:beef-ground]]`, `[[food:cooked-rice]]`, `[[food:eggs-in-shell]]`) and other guides as `[[guide:slug]]`. The site will render these as links. Use only slugs that exist in `src/data/base/foodkeeper.json` or the guide list you were given.
